/**
 * Sends chat messages to the MiSoul AI server (no memory dependency).
...
export const chatWithGPT = async (messages) => {
  const systemPrompt = `
You are misoul, a personal AI companion that helps users reflect on their thoughts and emotions.
...
"I’m still learning about you. Could you tell me more?"
  ...
  try {
    ...
    if (data.reply) {
      ...
      return "🤖 misoul couldn't generate a response.";
    }
  } catch (error) {
    console.error("🔥 GPT API Error (frontend):", error);
    return "⚠️ misoul is offline or unreachable.";
  }
};